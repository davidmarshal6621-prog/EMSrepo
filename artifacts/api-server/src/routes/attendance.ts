...
router.get("/attendance/summary", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const filtered = scopedEmployeeId ? records.filter(r => r.employeeId === parseInt(String(scopedEmployeeId), 10)) : records;
...
router.get("/attendance/today", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  let records = await db.select().from(attendanceTable).where(eq(attendanceTable.date, today));
...
router.get("/attendance", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const { employeeId, date, startDate, endDate, status, shiftId } = req.query as {
    employeeId?: string; date?: string; startDate?: string; endDate?: string; status?: string; shiftId?: string;
  };

  let records = await db.select().from(attendanceTable).orderBy(attendanceTable.date);

  const scopedEmployeeId = req.user?.role === "employee" ? req.user.employeeId : employeeId;
  if (scopedEmployeeId) records = records.filter(r => r.employeeId === parseInt(String(scopedEmployeeId), 10));
  ...
  if (shiftId) {
    const employeeIds = (await db.select({ id: employeesTable.id }).from(employeesTable)
      .where(eq(employeesTable.shiftId, parseInt(shiftId, 10)))).map(e => e.id);
    records = records.filter(r => employeeIds.includes(r.employeeId));
  }
...
router.post("/attendance", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  if (req.user?.role === "employee") {
    res.status(403).json({ error: "Employees cannot create attendance records" });
    return;
  }
...
router.patch("/attendance/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  if (req.user?.role === "employee") {
    res.status(403).json({ error: "Employees cannot edit attendance records" });
    return;
  }
...
router.delete("/attendance/:id", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  if (req.user?.role === "employee") {
    res.status(403).json({ error: "Employees cannot delete attendance records" });
    return;
  }
