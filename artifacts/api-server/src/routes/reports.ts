...
router.get("/reports/attendance-csv", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  ...
  const eid = req.user?.role === "employee" && req.user.employeeId
    ? req.user.employeeId
...
router.get("/reports/payslip-pdf", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const { employeeId, month, year } = req.query as { employeeId?: string; month?: string; year?: string };
  if (!employeeId) { res.status(400).json({ error: "employeeId required" }); return; }
...
router.get("/reports/payslip-check", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const { employeeId, month, year } = req.query as { employeeId?: string; month?: string; year?: string };
  if (!employeeId) { res.status(400).json({ error: "employeeId required" }); return; }
...
router.get("/reports/payroll-csv", requireAuth, async (req: AuthRequest, res): Promise<void> => {
  const now = new Date();
  ...
  const lines = ["Employee Code,Employee Name,Basic Salary,Allowances,Deductions,Net Salary,Present Days,Absent Days,Late Days,Status"];
  for (const r of records) {
    const emp = map.get(r.employeeId);
    const deductions = r.lateDeductions + r.leaveDeductions + r.otherDeductions;
    lines.push([
      emp?.employeeCode ?? "", `"${emp ? `${emp.firstName} ${emp.lastName}` : ""}"`,
      r.basicSalary, r.allowances, deductions, r.netSalary,
      r.presentDays, r.absentDays, r.lateDays, r.status,
    ].join(","));
  }
  res.setHeader("Content-Type", "text/csv");
