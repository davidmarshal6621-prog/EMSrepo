...
import { useListAttendance, useListEmployees, useListShifts } from "@workspace/api-client-react";
...
export default function AttendanceList() {
  ...
  const { data: attendanceData = [], isLoading } = useListAttendance({
    ...(startDate || endDate ? { startDate: startDate || undefined, endDate: endDate || undefined } : { date }),
    ...(shiftFilter !== "all" ? { shiftId: Number(shiftFilter) } : {}),
    ...(user?.role === "employee" && user.employeeId ? { employeeId: user.employeeId } : {}),
  } as any);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          ...
          {user?.role !== "employee" && <Button onClick={() => setShowAddModal(true)} className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-200 flex flex-col sm:flex-row gap-3 bg-gray-50/50">
          ...
          <div className="flex items-center gap-2">
            <Input type="date" aria-label="Start date" value={startDate} onChange={e => setStartDate(e.target.value)} className="bg-white w-full sm:w-40" />
            <span className="text-xs text-gray-500">to</span>
            <Input type="date" aria-label="End date" value={endDate} onChange={e => setEndDate(e.target.value)} className="bg-white w-full sm:w-40" />
          </div>
          <Select value={shiftFilter} onValueChange={setShiftFilter}>
            <SelectTrigger className="w-full sm:w-40 bg-white"><SelectValue placeholder="All shifts" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">All shifts</SelectItem>
              {(shifts as any[]).map(shift => <SelectItem key={shift.id} value={String(shift.id)}>{shift.name}</SelectItem>)}
            </SelectContent>
          </Select>
                ...
                filteredData.map((record) => (
                  <TableRow key={record.id} className={getStatusColor(record.status, record.isManuallyEdited)}>
                    ...
                    <TableCell className="text-right">
                      {user?.role !== "employee" && <Button variant="ghost" size="sm" onClick={() => openCorrect(record)}
